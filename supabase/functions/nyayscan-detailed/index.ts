...
${
  f.type === "text/plain"
    ? `Content: ${f.content}`
    : `[Binary file - analyze based on filename and type]`
}
`
  )
  .join("\n")}
...
`
    : caseType.toLowerCase().includes("property") ||
      caseType.toLowerCase().includes("real estate")
    ? `
...
`
    : caseType.toLowerCase().includes("employment") ||
      caseType.toLowerCase().includes("labour")
    ? `
...
`
    : caseType.toLowerCase().includes("family") ||
      caseType.toLowerCase().includes("divorce") ||
      caseType.toLowerCase().includes("matrimonial")
    ? `
...
Always respond with valid JSON only, no additional text or markdown.`;
    ...
    if (action === "start") {
      messages.push({
        role: "user",
        content:
          "Start the detailed analysis. Ask me relevant follow-up questions to understand my case better.",
      });
      ...
      messages.push({
        role: "user",
        content:
          "Generate the complete detailed analysis based on all the information I have provided.",
      });
    }
